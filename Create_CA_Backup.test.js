...
test('CA Submission', async ({ page }, testInfo) => {
  ...
  async function processCoverageDropdowns() {
    ...
    // DYNAMIC HEADER DETECTION: Auto-find all coverage section headers on the page
    // This eliminates the need to maintain a hardcoded list and catches new sections automatically
    let coverageHeaders = [];
    try {
      const detectStartTime = Date.now();
      
      // Strategy 1: Look for common coverage container patterns and extract headers
      // Common patterns: h3, h4, .coverage-header, .section-header, strong, etc. that precede dropdowns or buttons
      const headerSelectors = [
        'h3', 'h4', 'h5', '.coverage-header', '.section-header', '.panel-heading', '[data-coverage-header]'
      ];
        ...
        try {
          ...
          for (const el of elements) {
            try {
              ...
              // Filter out noise: skip if too short, too long, or contains common non-coverage text
              if (cleanText.length > 3 && cleanText.length < 100 && 
                  !cleanText.match(/^\s*(Select|Please|Next|Back|Save|Cancel|Close|Edit|Add|Remove|Click|Date|Form|Page)\s*$/i)) {
                
                // Check if this text has related dropdowns or buttons nearby (ensures it's a real coverage section)
                const parentContainer = el.locator('xpath=ancestor::*[contains(@class,"panel") or contains(@class,"card") or contains(@class,"section")][1]');
                const hasDropdown = await parentContainer.locator('select, [role="combobox"], .dropdown-toggle, button[data-action]').count().catch(() => 0);
                
                if (hasDropdown > 0) {
                  detectedHeaders.add(cleanText);
                  console.log(`    ✓ Detected coverage header: "${cleanText}" (has ${hasDropdown} interactive element(s))`);
                }
              }
            } catch (e) {
      ...
      coverageHeaders = Array.from(detectedHeaders);
      const detectTime = ((Date.now() - detectStartTime) / 1000).toFixed(2);
      console.log(`  📊 Auto-detected ${coverageHeaders.length} coverage section(s) in ${detectTime}s`);
      
      if (coverageHeaders.length === 0) {
        console.log(`  ⚠️  No coverage headers auto-detected. Using fallback hardcoded list...`);
        // Fallback: use hardcoded list if dynamic detection fails
        coverageHeaders = ['Liability', 'Uninsured Motorists Coverage', 'Uninsured Motorists Property Damage', 'PIP Coverage', 'Added Personal Injury Protection', 'Private Passenger Liability', 'Private Passenger Other Than Collision', 'Private Passenger Collision', 'Private Passenger Towing Labor', 'Private Passenger Uninsured Motorists Property Damage', 'Private Passenger Uninsured Motorists', 'Private Passenger Underinsured Motorists','Private Passenger Personal Injury Protection','Truck Liability','Truck Other Than Collision','Truck Collision','Truck Towing Labor','Truck Uninsured Motorists Property Damage','Truck Uninsured Motorists','Truck Underinsured Motorists','Rental Reimbursement','Private Passenger Michigan Catastrophic Surcharge','Comprehensive Coverage','Collision Coverage','Truck Personal Injury Protection','Added Personal Injury Protection - Truck', 'Business Use Coverage', 'Hired Auto Liability', 'Non-Owned Auto Liability', 'Garage Liability', 'Garagekeepers Coverage', 'Employers Non-Owned Auto Liability', 'Truck Michigan Auto Theft Prevention Authority Fee','Private Passenger Michigan Catastrophic Statutory Fee','Truck Michigan Auto Theft Prevention Authority Fee','Truck Michigan Catastrophic Statutory Fee'];
      }
    } catch (e) {
      console.log(`  ⚠️  Error during dynamic header detection: ${e.message.split('\n')[0]}. Using fallback list...`);
      // Fallback to hardcoded list on error
      coverageHeaders = ['Liability', 'Uninsured Motorists Coverage', 'Uninsured Motorists Property Damage', 'PIP Coverage', 'Added Personal Injury Protection', 'Private Passenger Liability', 'Private Passenger Other Than Collision', 'Private Passenger Collision', 'Private Passenger Towing Labor', 'Private Passenger Uninsured Motorists Property Damage', 'Private Passenger Uninsured Motorists', 'Private Passenger Underinsured Motorists','Private Passenger Personal Injury Protection','Truck Liability','Truck Other Than Collision','Truck Collision','Truck Towing Labor','Truck Uninsured Motorists Property Damage','Truck Uninsured Motorists','Truck Underinsured Motorists','Rental Reimbursement','Private Passenger Michigan Catastrophic Surcharge','Comprehensive Coverage','Collision Coverage','Truck Personal Injury Protection','Added Personal Injury Protection - Truck', 'Business Use Coverage', 'Hired Auto Liability', 'Non-Owned Auto Liability', 'Garage Liability', 'Garagekeepers Coverage', 'Employers Non-Owned Auto Liability', 'Truck Michigan Auto Theft Prevention Authority Fee','Private Passenger Michigan Catastrophic Statutory Fee','Truck Michigan Auto Theft Prevention Authority Fee','Truck Michigan Catastrophic Statutory Fee'];
    }

    // Find all coverage sections by headers (now dynamically detected or fallback to hardcoded)
      ...
      try {
        ...
        for (let idx = 0; idx < selectElements.length; idx++) {
          try {
            ...
            // OPTIONAL: Try to find paired BUTTON (usually next sibling) - not required for direct selectOption()
            let nextBtn = null;
            let btnExists = false;
            let btnVisible = false;
            try {
              nextBtn = await select.locator('xpath=following-sibling::button[1]').first();
              btnExists = await nextBtn.count().catch(() => 0);
              if (btnExists > 0) {
                btnVisible = await nextBtn.isVisible().catch(() => false);
                console.log(`    Candidate ${idx + 1}: SELECT id=${selectInfo.id} with ${selectInfo.optionCount} options - paired button ${btnVisible ? 'visible' : 'found but hidden'}`);
              } else {
                console.log(`    Candidate ${idx + 1}: SELECT id=${selectInfo.id} with ${selectInfo.optionCount} options - no paired button (will use direct selectOption)`);
              }
            } catch (e) {
              console.log(`    Candidate ${idx + 1}: SELECT id=${selectInfo.id} with ${selectInfo.optionCount} options - button search failed, proceeding without button`);
            }
            
            // Add dropdown regardless of button availability - we'll use direct selectOption()
        ...
        for (let j = 0; j < Math.min(dropdowns.length, maxPerSection); j++) {
          try {
            ...
            try {
              ...
              await page.waitForTimeout(2500); // Allow select change to register fully
              ...
              await page.waitForTimeout(2000); // Give DOM time to process change event
              ...
              if (j < Math.min(dropdowns.length, maxPerSection) - 1) {
                ...
                await page.waitForTimeout(4000); // Extra time for previous dropdown to persist
              }
            } catch (e) {
        ...
        // Helper function to retry Save button click with up to 3 attempts
        const clickSaveWithRetry = async (buttonLocator, context = 'modal', maxRetries = 3) => {
          let lastError = null;
          for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
              const saveButton = buttonLocator.first();
              const saveVisible = await saveButton.isVisible().catch(() => false);
              if (!saveVisible) {
                throw new Error('Save button not visible');
              }
              console.log(`  💾 Attempt ${attempt}/${maxRetries}: Clicking Save button (${context})...`);
              await saveButton.click({ timeout: 5000 });
              await page.waitForTimeout(800); // Brief wait to see if click registered
              console.log(`  ✅ Save button clicked successfully on attempt ${attempt}`);
              await page.waitForTimeout(1500); // Wait for modal/section to process
              return true; // Success
            } catch (e) {
              lastError = e;
              console.log(`  ⚠️  Attempt ${attempt}/${maxRetries} failed: ${e.message.split('\n')[0]}`);
              if (attempt < maxRetries) {
                await page.waitForTimeout(500); // Brief pause before retry
              }
            }
          }
          console.log(`  ❌ Failed to click Save button after ${maxRetries} attempts (${context}): ${lastError?.message || 'unknown error'}`);
        ...
        if (modalOpened) {
          ...
          try {
            ...
            const saveClicked = await clickSaveWithRetry(modalSaveButton, 'modal', 3);
            if (saveClicked) {
              console.log(`  💾 Edit Coverage modal closed successfully`);
            }
          } catch (e) {
          ...
          try {
            const saveButton = coverageSection.locator('button:has-text("Save"), button[title*="Save"]');
            const saveExists = await saveButton.count().catch(() => 0);
            
            if (saveExists > 0) {
              console.log(`  💾 Found Coverage Section Save button, attempting to click...`);
              const saveClicked = await clickSaveWithRetry(saveButton, 'coverage section', 3);
              if (saveClicked) {
                console.log(`  💾 Save button clicked and processing complete`);
              }
            }
          } catch (e) {
    ...
    // NOTE: Milestone creation moved outside this function to avoid duplicates
    // since this function is called multiple times (once per vehicle).
    // Milestones will be created once at the end after all vehicles are processed.
