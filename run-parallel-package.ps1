...
# Align with BOP runner: full-state defaults, lock handling, batch markers, resource logs
...
# Optionally stop stray Playwright runs
if ($KillStrays) {
    try {
...
# Init lock, clean artifacts, ensure batch marker
try {
...
while ($pendingStates.Count -gt 0 -or $activeProcs.Count -gt 0) {
    ...
    foreach ($procInfo in $activeProcs) {
        if (-not $procInfo.proc.HasExited) {
            ...
            $duration = $null
            if ($procInfo.startTime) { $duration = ($endTime - $procInfo.startTime).TotalSeconds }
            $procStats = $null
            try { $procStats = Get-Process -Id $procInfo.proc.Id -ErrorAction SilentlyContinue } catch {}
            $cpu = $procStats.CPU
            ...
            $logBuffer += "    [$state] Package test completed (ExitCode: $exitCode, Duration: $duration s, CPU: $cpu, RAM: $mem)"
