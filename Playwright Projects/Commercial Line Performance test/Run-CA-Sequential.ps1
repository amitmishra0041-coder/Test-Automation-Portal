# Run Create_CA_FinalCopy tests in parallel (max 2 workers, 60s stagger, always headed)
param(
    [string]$TestEnv = "qa",
    [string[]]$States,
    [switch]$NoEmail,
...
$lockFile       = Join-Path $workingDir 'parallel-run-lock-ca.json'
$iterationsFile = Join-Path $workingDir 'iterations-data-ca.json'
$batchMarker    = Join-Path $workingDir '.batch-run-in-progress-ca'
$batchEmailSent = Join-Path $workingDir '.batch-email-sent-ca'
...
while ($pendingStates.Count -gt 0 -or $activeProcs.Count -gt 0) {
    ...
    foreach ($procInfo in $activeProcs) {
        if (-not $procInfo.proc.HasExited) {
            ...
            $log      = if ($stateLogs[$state]) { $stateLogs[$state] } else { @() }
    ...
    $enoughTimeElapsed = $isFirstRun -or ([int]([DateTime]::Now - $lastStartTime).TotalSeconds -ge 60)
    ...
    if ($canStart -and $enoughTimeElapsed) {
        ...
        $stateLogs[$state] = @("[$state] Starting at $iterationStart (active workers: $($activeProcs.Count + 1))")

        Write-Host "[$state] Launching headed browser..." -ForegroundColor Cyan

        $outDir = Join-Path $workingDir "test-results\ca-$state"
        ...
        $envCmd = 'set "TEST_STATE=' + $state + '" && set "TEST_ENV=' + $TestEnv + '" && set "TEST_TYPE=CA" && npx playwright test .\Create_CA_FinalCopy.test.js --project=chromium --workers=1 --headed --output="' + $outDir + '" & set "PW_EXIT=%ERRORLEVEL%" & echo %PW_EXIT% > "' + $exitCodeFile + '" & exit /b %PW_EXIT%'
        ...
        # Sleep briefly then continue so the 60s gap is respected for the next launch
...
if ($failed -eq 0) {
    Remove-Item (Join-Path $workingDir 'iterations-data-ca.json')  -Force -ErrorAction SilentlyContinue
