...
REM Usage: run-bop-test.bat [qa|test|prod] [states] [headed]
...
if %HAS_HEADED%==1 (
    echo Running BOP Test in Chromium (HEADED mode) for STATES: %STATES% in PARALLEL...
) else (
...
if %HAS_HEADED%==1 (
    ...
    powershell.exe -ExecutionPolicy Bypass -File "%~dp0run-parallel-bop.ps1" -TestEnv %ENV% -States "%STATES%" -Headed -Project chromium -KillStrays
) else (
    powershell.exe -ExecutionPolicy Bypass -File "%~dp0run-parallel-bop.ps1" -TestEnv %ENV% -States "%STATES%" -Project chromium -KillStrays
)
